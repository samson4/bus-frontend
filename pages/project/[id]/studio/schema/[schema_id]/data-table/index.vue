...
const dbTable = ref(props.selectedTable);
...
const fetchTableData = async () => {
  ...
  const columnsResponse = await axios.get(
    `/columns/?table=${props.selectedTable}&schema=${route.params.schema_id}`
  );
  ...
  const dataResponse = await axios.get(
    `/data/?schema=${route.params.schema_id}&table=${props.selectedTable}&limit=${dataLimit.value}&skip=${dataOffset.value}`
  );
