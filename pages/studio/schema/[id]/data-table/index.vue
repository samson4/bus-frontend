...
const fetchTableData = async () => {
  ...
  const columnsResponse = await axios.get(
    `/columns/?table=${props.selectedTable}&schema=${route.params.id}`
  );
  ...
  const dataResponse = await axios.get(
    `/data/?schema=${route.params.id}&table=${props.selectedTable}&limit=${dataLimit.value}&skip=${dataOffset.value}`
  );
